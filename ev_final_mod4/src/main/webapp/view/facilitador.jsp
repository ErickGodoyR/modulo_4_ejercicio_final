...
<title>LISTA DE FACILITADOR</title>
...
<h1 class="text-sm-center text-md-center text-lg-center text-xl-center fs-3 text-decoration-underline">LISTA DE FACILITADORES</h1>
	
	<div class="row">
    	<div class="col align-self-center mb-4">
    		<a href="${pageContext.request.contextPath}/AgregarFacilitadorServlet"
			class="btn btn-primary">Nuevo Facilitador</a>		
    	</div>
	</div>
			...
			<tbody>
				<c:forEach var="fa" items="${facilitador}">
					<tr>
						...
						<td> <c:out value = "${fa.getValorhora()}"/></td>
						<td><c:out value="${fa.getBanco()}"></c:out></td>
						<td><c:out value="${fa.getCtabancaria()}"></c:out></td>
						<td> <c:out value = "${fa.getLast_update()}"/></td>
						<td>
							<a
							href="${pageContext.request.contextPath}/AgregarFacilitadorServlet?id=${fa.getId()}"
							class="btn btn-secondary"><i class="fa-solid fa-pen-to-square"></i></a>
						</td>
						<td>
							<a
							href="${pageContext.request.contextPath}/AgregarFacilitadorServlet?id=${fa.getId()}&op=del"
